        ...
        function loadPage(hash) {
            ...
            if (hash === '#/updater') {
                ...
                $('#pageContent').html(`
                    <div class="h-[calc(100vh-10rem)] w-full">
                        ...
                        <div class="mt-4 flex gap-4">
                            <button id="updateCodeBtn" disabled class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded flex items-center gap-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                                <i class="mdi mdi-upload"></i> Update
                            </button>
                ...
                require(['vs/editor/editor.main'], function() {
                    Promise.all(files.map(f => fetch(f.url).then(r => r.text())))
                        .then(contents => {
                            ...
                            $('#updateCodeBtn').on('click', function() {
                                ...
                                $.ajax({
                                    ...
                                    data: JSON.stringify(modifiedContents),
                                    success: function() {
                                        alert('Code sent for update!');
                                        // Update original contents after successful save
                                        Object.keys(modifiedContents).forEach(key => {
                                            originalContents[key] = modifiedContents[key];
                                        });
                                        updateGitDecorations();
                                        updateButtonStates();
                                    },
